...
<div class="container">
	...
	<br><br>
	<form class="form-horizontal" action="/good/goodUpdate" method="post" enctype="multipart/form-data" onsubmit="return check()">
		...
		<div class="form-group">
			...
			<div class="col-sm-6"><img src="${good.cover}" width="100"/>
				<input type="file" name="file" id="input_file" value="${good.cover}" style="display:inline;"> ( 推荐尺寸: 500 * 500 )
			</div>
		</div>
		<div class="form-group">
			...
			<div class="col-sm-6">
				<select class="form-control" id="select_topic" name="type_id">
					<c:forEach var="type" items="${typeList}">
						<c:if test="${type.id==good.type_id}"><option selected="selected" value="${type.id}">${type.name}</option></c:if>
						<c:if test="${type.id!=good.type_id}"><option value="${type.id}">${type.name}</option></c:if>
					</c:forEach>
				</select>
			</div>
		</div>
